...
const Register = () => {
  ...
  const inputClass = "w-full pl-8 pr-3 py-2.5 bg-white/[0.06] border border-white/10 rounded-xl text-white/80 text-sm outline-none placeholder:text-white/25 focus:border-white/20 transition-colors";
      ...
      >
        <div>
          <p className="text-white/85 text-lg font-semibold">
            {isGetOTP ? "Verify your email" : "Create your account"}
          </p>
          <p className="text-white/35 text-sm mt-0.5">
            {isGetOTP ? `OTP sent to ${email}` : "Start hosting events on myPass"}
          </p>
        </div>
        ...
        {!isGetOTP && (
          <>
            <div className="relative flex items-center">
              <User size="0.85rem" className="absolute left-3 text-white/30 pointer-events-none" />
            ...
            <div className="relative flex items-center">
              <Phone size="0.85rem" className="absolute left-3 text-white/30 pointer-events-none" />
